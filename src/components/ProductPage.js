...
const ProductPage = () => {
  ...
  return (
    <>
      ...
      <div className="w-full h-full bg-white p-8 rounded-lg shadow-md  flex mx-auto  ">
        <div className="flex mx-auto max-md:flex-col">
          <div className="w-96 relative overflow-hidden group  max-sm:w-full">
            <img
              src={item.itemImage}
              alt="imagehere"
          ...
          <div >
            ...
            <p className="text-lg text-gray-700 font-medium mt-2">
              {`$${item.itemPrice}`} & FREE DILEVERY
            </p>
            <button
              className="bg-black text-white w-28 h-12 border border-white"
              onClick={() => handleAddToCart1(item)}
            >
              Add to Cart
            </button>
            <p className="text-gray-600 mt-2 w-full md:w-96"><b>Description</b><br></br>{item.itemDescription}</p>
          </div>
        </div>
      </div>
      <Footer></Footer>
